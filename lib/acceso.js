 ...
 *
 * Empezó en 40 pensando en un fisioterapeuta usándola en consulta, y resultó
 * ser incómodo incluso para eso: rehacer una patología entera son quince o
 * veinte consultas, y probando la herramienta se agota en una tarde.
 *
 * Con 0 no hay límite personal. Sigue habiendo cupo global, que es el que de
 * verdad protege la factura.
 */
export const LIMITE_DIARIO = Number(process.env.LIMITE_DIARIO ?? 200);

/** Tope de toda la aplicación por día. Cortafuegos de la factura. Con 0, ninguno. */
...
export async function anotarConsulta(registro) {
  ...
  if (CUPO_GLOBAL > 0 && global.consultas >= CUPO_GLOBAL) {
    return { permitido: false, motivo: "cupo_global" };
  }

  const usadas = registro.consumo?.[dia] || 0;
  if (LIMITE_DIARIO > 0 && usadas >= LIMITE_DIARIO) {
  ...
  return { permitido: true, restantes: LIMITE_DIARIO > 0 ? LIMITE_DIARIO - usadas - 1 : null };
}
