...
/**
 * Vacía un almacén entero. Devuelve cuántos elementos había.
 *
 * Existe para poder rehacer la base de evidencia cuando cambia la forma de
 * buscarla: lo guardado se congela con su cita a propósito, así que una mejora
 * en la búsqueda no alcanza a lo que ya está dentro si no se tira.
 */
export async function vaciar(nombre) {
  if (enNetlify) {
    const almacen = await almacenBlobs(nombre);
    const { blobs } = await almacen.list();
    for (const { key } of blobs) await almacen.delete(key);
    return blobs.length;
  }
  try {
    const carpeta = path.join(DIRECTORIO, nombre);
    const archivos = (await fs.readdir(carpeta)).filter((a) => a.endsWith(".json"));
    for (const a of archivos) await fs.unlink(path.join(carpeta, a));
