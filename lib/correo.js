...
//
// Dos proveedores tras la misma función porque la cuenta puede estar en
// cualquiera de los dos: se elige con PROVEEDOR_CORREO, y por defecto se usa
// el que tenga clave configurada. Cambiar de uno a otro no toca código.
...
export async function enviarAcceso({ correo, enlace }) {
  ...
  const asunto = "Tu acceso al Asistente de Tests de FisioIA";

  const texto = [
    "Ya tienes acceso al Asistente de Tests.",
    "",
    "Entra desde aquí:",
    enlace,
    "",
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f5f7fb;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:14px;padding:36px 32px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#0f2740;">
        ...
        <tr><td style="font-size:20px;font-weight:700;line-height:1.3;padding-bottom:14px;">Ya tienes acceso</td></tr>
        ...
        <tr><td align="center" style="padding-bottom:26px;">
          <a href="${enlace}" style="display:inline-block;background:#00a97f;color:#ffffff;text-decoration:none;font-weight:600;font-size:16px;padding:14px 30px;border-radius:10px;">Abrir el asistente</a>
        </td></tr>
